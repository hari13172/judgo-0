...
function App() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-screen">Loading Problems...</div>
  }
      ...
      {checkingDisplays && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-background p-6 rounded-lg shadow-lg max-w-md text-center">
            <Monitor className="h-12 w-12 text-primary mx-auto mb-4 animate-pulse" />
            <h2 className="text-xl font-bold mb-2">Checking Display Configuration</h2>
            <p className="text-muted-foreground mb-4">
              Please wait while we verify your display setup for test security...
            </p>
      ...
      {displayCheckPassed && !showProblemContent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-background p-6 rounded-lg shadow-lg max-w-md text-center">
            ...
            <p className="text-muted-foreground mb-4">Click the button below to start your test in fullscreen mode.</p>
            <Button onClick={handleStartTest} className="bg-primary hover:bg-primary/90">
              Start Test
            </Button>
          </div>
        </div>
